...
from src.util.chebyshev_diff import chebyshev_diff_matrix
...
if __name__ == "__main__":
    ...
    def loss_function(params):
        U, l = params
        return 1/n_y * np.sum((-l*U + ((1+l)*y + U)*(D@U))[1:-1]**2) + 1/2*((U[-1]-1)**2 + (U[0]+1)**2) + np.sum((D4@U)[diff_reg_msk]**2)

    U0 = -y / 2.0
    bounds = [(None, None)] * len(U0) + [(0.3, 0.7)]
    x0 = np.concatenate([U0, [0.3]])
    def loss_function_wrapped(x):
        U = x[:-1]
        l = x[-1]
        return loss_function((U, l))
    result = minimize(loss_function_wrapped, x0=x0, bounds=bounds, method='SLSQP', tol=1e-8, options={"maxiter": 100000})
