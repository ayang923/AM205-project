...
Although the numerical higher order derivatives (particularly the third and fourth) show instabilities near the boundaries, the method nevertheless resolves the key features distinguishing the two solutions: the cusp at the origin when $\lambda = 0.4$ and the smooth profile for $\lambda = 0.5$, achieving accuracies comparable to the PINN-based method reported in~\cite{wang2023asymptotic}. The results demonstrate that the piecewise Chebyshev collocation method, combined with gradient-based optimization, provides an effective and accurate approach for solving the boundary value problem~\eqref{eq:bvp}, even in the presence of non-smooth behavior at the origin. The method successfully handles both smooth and non-smooth cases, with the natural clustering of Chebyshev points near the origin ensuring adequate resolution where it is most needed.
...
As mentioned in the preceding sections, our goal is to single out the smooth solution at $\lambda = 0.5$ from nearby non-smooth solutions by exploiting the fact that the non-smooth solutions have discontinuous fourth derivative at the origin. To do so, we consider the new residual function $\widehat{\mathcal{L}}_\lambda: \mathbb{R}^{n_y/2+1}\times\mathbb{R}^{n_y/2+1}\times \mathbb{R} \to \mathbb{R}^{n_y+3}$ given by 
\begin{equation}
    \widehat{\mathcal{L}}(\mathbf{U}_1, \mathbf{U}_2, \lambda) = 
    \begin{pmatrix}
        \mathcal{L}_\lambda(\mathbf{U}_1, \mathbf{U}_2)\\
        \mathcal{L}^{\textrm{reg}}_\lambda(\mathbf{U}_1, \mathbf{U}_2)
    \end{pmatrix}
\end{equation}
where $\mathcal{L}_\lambda(\mathbf{U}_1, \mathbf{U}_2)$ is defined in eq.~\eqref{eq:fixed_l_loss}, and 
\begin{equation}
    \mathcal{L}^{\textrm{reg}}_\lambda(\mathbf{U}_1, \mathbf{U}_2) = 
        (D^4\mathbf{U}_1)_{n_y/2} - (D^4\mathbf{U}_2)_{0}.
\end{equation}
penalizes any jump in the fourth derivative.

In the PINN framework in~\cite{wang2023asymptotic}, the optimization is performed simultaneously over both the parameter $\lambda$ and the solution $U(y)$, where the solution is represented implicitly by neural network weights. We attempted to adopt the same strategy with out collocation-based approach, but we found that the joint optimization either converged to incorrect solutions or failed to converge altogether. We believe this difficulty reflects a fundamental difference in the two formulations. In the PINN setting, the optimization variables are network weights--parameters that do not directly corresond to pointwise values of the solution. The neural network itself provides a form of regularization, thereby improving conditioning for the problem. On the other hand, our approach optimizes directly over the solution values at the Chebyshev collocation points, rather than over network weights, introducing severe ill-conditioning when attempting to jointly optimize $\lambda$ and $U$ together. The fundamental issue arises from the fact that the fourth derivative operator $D^4$ is extremely ill-conditioned, with a condition number that grows rapidly with the number of collocation points. When we include the regularization term $\mathcal{L}^{\textrm{reg}}_\lambda(\mathbf{U}_1, \mathbf{U}_2)$ that enforces continuity of the fourth derivative at the interface, the overall problem becomes extremely ill-conditioned, making gradient-based optimization methods (such as Newton-type methods) numerically unstable and prone to convergence failures. This behavior is also intuitive: even very small perturbations in discrete solution values can produce extremely large errors in high-order derivatives, making simultaneous optimization over $\lambda$ and $U$ extremely difficult.

To overcome this challenge, we adopt a nested optimization strategy: we construct a grid of candidate values for $\lambda$, solving the fixed-$\lambda$ problem described in Section~\ref{subsec:optimization} for each $\lambda$ to obtain the corresponding solution $U_\lambda(y)$. We then evaluate the regularization residual $\mathcal{L}^{\textrm{reg}}_\lambda(\mathbf{U}_1, \mathbf{U}_2)$ as a function of $\lambda$ alone, and optimize this scalar function using the Nelder-Mead method (a derivative-free optimization algorithm). This decoupled approach avoids the ill-conditioning issues associated with the joint optimization, as the inner optimization (over $U$ for fixed $\lambda$) remains well-conditioned, while the outer optimization (over $\lambda$) operates on a scalar function that can be reliably minimized using derivative-free methods.
